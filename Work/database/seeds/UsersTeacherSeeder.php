...
class UsersTeacherSeeder extends Seeder
{
    ...
    public function run()
    {
        $data = [
            ...
            [
                ...
                "email" => "e-mail  a.a.sitnikova@mpt.ru"
            ],
            [
                "name" => "Александр",
                "secName" => "Сачков",
        ...
        foreach($data as $key => $value) 
        {
            if($data[$key]['email'] == null)
                $data[$key]['email'] = $this->translit($data[$key]['name'])[0].'.'.$this->translit($data[$key]['thirdName'])[0].'.'.$this->translit($data[$key]['name']).'@mpt.ru';
            $data[$key]['post_id'] = 3;
            $data[$key]['password'] = Hash::make("secret");
