...
class HomeWorkStudentRepository extends BaseRepository
{
    ...
    public function getHomeWorkStudentById($association_home_work_id, $student_id)
    {
        ...
        $result = $this->startCondition()->where([['association_home_work_id', $association_home_work_id],['student_id', $student_id]])->select($columns)->orderBy('id', 'desc')->toBase()->get();
    ...
    public function getHomeWorkStudent($association_home_work_id)
    {
        $columns = ['id', 'student_id', 'info', 'created_at as date'];
        $result = $this->startCondition()->where([['association_home_work_id', $association_home_work_id]])->select($columns)->orderBy('id', 'desc')->toBase()->get();
