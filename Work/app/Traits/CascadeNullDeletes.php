...
trait CascadeNullDeletes
{
     ...
     */
    protected static function bootCascadeNullDeletes()
    {
        static::deleting(function ($model) {
            ...
            $cascade = $model->getActiveCascadingDeletes();


            foreach ($cascade as $relationship => $value) {
                if ($model->{$relationship} instanceof Model) {
                    $relat = $model->{$relationship};
                    $relat->{$value} = NULL;
                    $relat->save();
                } else {
                    Debugbar::info($value);
    ...
    protected function hasInvalidCascadingRelationships()
    {
        ...
        return array_filter($this->getCascadingDeletes(), function ($key,$relationship) {
