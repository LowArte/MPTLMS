        ...
        v-layout.column.wrap
            ...
            v-flex(v-if="journal")
                v-card.mx-auto.pa-2(raised dark min-width="300px")
                    ...
                    v-btn(small dark color="red" @click="editJournal") сохранить журнал
                        ...
                        tbody(v-if="journal")
                            tr(v-for="(item, i) in journal.students" :key="item.name")
                                ...
                                td(class="text-center" v-for="ozenka in journal.journal[selected_semester]" :key="journal.key")
                                    v-menu(bottom origin="center center" transition="scale-transition")
                                        template(v-slot:activator="{ on }")
                                            v-btn(small text v-on="on" outlined :color="ozenka.students[item.id] == '2' ? 'red' : ozenka.students[item.id] == '3' ? 'orange' : ozenka.students[item.id] == '4' ? 'light-green' : ozenka.students[item.id] == '5' ? 'green' : 'white'") {{ozenka.students[item.id]}}
                                        v-list
