...
export default {
    ...
    methods:{
        departament_change() //Получение групп при изменении отдела
        {
            group_api
        ...
        caseDate() //При выборе даты получать расписание выбранного дня
        {
            var date = new Date(this.dateDialog.date);
            if(date.getDay() == 0)
            {
                this.dateDialog.date = new Date().toISOString().substr(0, 10);
                alert("На воскресенье нет расписания!");
            }
            else
            schedule_api
        ...
        isChisl() //Определения числителя/знаменателя недели
        {
            const today = new Date(this.dateDialog.date);
            return today.getWeek() % 2;
        },
        sendQuery() //Сохранение замены
        {
            ...
            .then(res => {
                alert('Замена сохранена!');
            })
            .catch(ex => {
                console.log(ex);
                alert("Замена не принята!");
            });
        },
        caseLesson(number)
        {
